...
/**
 * create a 1-indexed priority queue(no order specified) instance with given
 * capacity, return a pointer to instance
 * @cap - capacity of priority queue
 */
pq *pq_new() {
...
/**
 * make current node and its child nodes a max root heap
 * @list - number list (1-indexed)
 * @len - list length
 * @iCurr - index of current node
 */
void pq_max_heapify(PQ_DATA_TYPE *list, int len, int iCurr) {
  int iMax = iCurr;
  int iLC = iCurr * 2; // left child
  int iRC = iLC + 1;
  // find index of the max node in following heap
  if (iLC <= len && list[iMax] < list[iLC])
    iMax = iLC;
  if (iRC <= len && list[iMax] < list[iRC])
    iMax = iRC;
  // if current isn't max, swap with its max child
  if (iMax != iCurr) {
    pq_swap(&list[iCurr], &list[iMax]);
    // swap may destroy child's heap, so recursion is needed
    pq_max_heapify(list, len, iMax);
  }
}

/**
 * add/push a new value to max priority queue
 * @ins - max priority queue instance(1-indexed)
 * @val - value to add
 */
void pq_max_push(pq *ins, PQ_DATA_TYPE val) {
  ...
  int i = ins->len; // node's index
  int iF = i / 2;   // father node's index
...
/**
 * pop/delete root node from max priority queue
 * @ins - max priority queue(1-indexed)
 */
